...
export default function EditPatientPage() {
  ...
  // Initial State (Simulating data fetch)
  ...
  const handleSave = (e) => {
    e.preventDefault();
    console.log("Updating Patient:", params.id, formData);
      ...
      <div className="content-card" style={{ marginTop: '24px', maxWidth: '800px' }}>
        ...
        <div style={{ marginBottom: '30px', borderBottom: '1px solid #eee', paddingBottom: '15px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
                ...
                <span style={{ fontSize: '13px', color: '#666' }}>Editing details for ID: <strong>{params.id}</strong></span>
            </div>
            {/* Delete/Archive Button */}
            <button style={{ background: '#fee2e2', color: '#991b1b', border: 'none', padding: '6px 12px', borderRadius: '6px', fontSize: '12px', fontWeight: 'bold', cursor: 'pointer' }}>
                Archive Account
            </button>
        </div>
